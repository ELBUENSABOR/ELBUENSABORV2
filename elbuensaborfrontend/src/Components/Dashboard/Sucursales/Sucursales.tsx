...
import {deleteSucursal, fetchSucursales} from "../../../services/dashboardService";
...
const Sucursales = () => {
    ...
    const handleDeleteSucursal = async (id: number) => {
        const confirmed = window.confirm("¿Seguro que querés eliminar esta sucursal?");
        if (!confirmed) return;

        try {
            await deleteSucursal(id);
            setSucursales((prev) => prev?.filter((sucursal) => sucursal.id !== id));
            setOriginalSucursales((prev) => prev?.filter((sucursal) => sucursal.id !== id));
        } catch (error) {
            console.error("Error al eliminar sucursal", error);
            const axiosError = error as AxiosError<{ error?: string; mensaje?: string }>;
            const backendMessage = axiosError.response?.data?.error ?? axiosError.response?.data?.mensaje;
            alert(backendMessage ?? "No se pudo eliminar la sucursal");
        }
    };
                                ...
                                <td>
                                    <div className="d-flex gap-2">
                                        ...
                                        <button
                                            onClick={() => {
                                                const id = s.id;
                                                if (id != null) handleDeleteSucursal(id);
                                            }}
